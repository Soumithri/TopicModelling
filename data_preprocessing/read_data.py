...
def main():
    ...
    with open(dir + '/' + DIRECTORY + '_raw_username.csv', 'w') as raw_u,\
        ...
        print('opened directories...')
        for user in user_list:
            print(user)
            total_users +=1
            if total_users % 50 ==0:
                logging.debug("User processed : {0}".format(total_users))
            tweets, u_tweets = get_tweet_preprocessed(coll, str(user))
            #print('*******',tweets)
            if len(tweets) < 6:
                discarded += 1
                continue
...
def get_tweet_preprocessed(coll, user):
    print('In tweet: ', 'user: ', user)
    query ={}
    query['user.id_str'] = user
    query['lang'] = 'en'
    projection = {'user.id_str' : 1,  'text': 1}
    ...
    print('count: ', count, 'len of tweet_list: ', len(tweet_list))
    #print(tweet_list)
...
def get_mongo_connection(host="localhost", port=27017, db_name="tweetCorpus"):
